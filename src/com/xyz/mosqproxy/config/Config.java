...
public class Config
{
	...
	/**
	 * 读入配置文件，不准以追加的方式加载，并且配置文件已经被加载的时候就直接返回
	 * @param configFile 配置文件的路径
	 * @param append	 是否以追加的方式加载配置文件
	 * @throws Exception
	 */
	public boolean loadConfig(String configFile, boolean append)
	{
		/*不准以追加的方式加载，并且配置文件已经被加载的时候就直接返回*/
		if (!append && m_contents.size() > 0)
